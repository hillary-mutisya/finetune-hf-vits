#!/usr/bin/env python3
"""
Generate comparison samples: v2 (50 epochs) vs production vs v1 (200 epochs)
"""
...
MODELS = {
    "v2_50ep": "mutisya/vits_luo_26_05_f_v2",
    "production": "mutisya/vits_luo_drL_24_5-v24_27_1_f",
    "v1_200ep": "mutisya/vits_luo_26_05_f_v1",
}

NOISE_CONFIGS = {
    "optimized": {"noise_scale": 0.1, "noise_scale_duration": 0.0},
    "default": {"noise_scale": 0.667, "noise_scale_duration": 0.8},
}
...
def compute_noise_floor(waveform, sr):
    ...
    energies.sort()
    # Noise floor = average of bottom 10% of frame energies
    n = max(1, len(energies) // 10)
    return np.mean(energies[:n])

def main():
    device = "cuda" if torch.cuda.is_available() else "cpu"
    output_dir = Path("luo_v2_samples")
    ...
    print(f"\n{len(TEXTS) * len(MODELS) * len(NOISE_CONFIGS)} audio files saved to {output_dir}/")
